...
if (isset($_POST['btnsave'])) {
    ...
    if ($cate_name == "") {
        ...
        $err = "<span style='color:red;'>Bắt buộc phải nhập tên chuyên mục</span>";
    } else {
        $cate_name = $_POST["cate_name"];
        $countt = mb_strlen($name); // Đếm số lượng ký tự của tên chuyên mục
        if (preg_match("/^[a-zA-Z ]*$/", $cate_name) == false && $countt < 3) {
            // Kiểm tra nếu không thỏa mãn thì báo lỗi
            $err = "<span style='color:red;'>Tên chuyên mục ít nhất 3 ký tự</span>";
        } else {
            $sql = "SELECT * FROM cate_post WHERE cate_name='$cate_name'";
            $query = mysqli_query($connect, $sql);
            $row = mysqli_fetch_array($query);
            if ($row["cate_name"] == "" || $row['id'] == $id) {
                ...
                $cate_name = $_POST['cate_name'];
                if (isset($cate_name)) {
                    $sql = "UPDATE cate_post SET cate_name='$cate_name' WHERE id=$id";
                    $query = mysqli_query($connect, $sql);
